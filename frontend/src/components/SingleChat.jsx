...
function SingleChat({ fetchAgain, setFetchAgain }) {
    ...
    return (
        <div className='h-full flex flex-col'>
            {selectedChat ? (
                <>
                    <div className='flex justify-between text-white text-2xl p-3'>
                        ...
                        {!selectedChat.isGroupChat ? (
                            <>
                                ...
                                <div className='cursor-pointer' onClick={toggleProfileModal}>
                                    <FontAwesomeIcon icon={faEye} />
                                </div>
                                <div ref={profileModalRef} className='modal user-profile text-black'>
                                    {showProfile && <ProfileModal user={getSenderFull(user, selectedChat.users)} />}
                                </div>
                            </>
                        ) : (
                            <>
                    ...
                    {loading ? <div className='grid h-full place-content-center'><Spinner h={20} /></div> :
                        <div className='bg-[url("https://e0.pxfuel.com/wallpapers/933/463/desktop-wallpaper-dark-space-whatsapp-dark.jpg")] bg-contain flex-grow p-2 messages' ref={chatContainerRef}>
                            <div className='text-black'>
                                <ScrollableChat messages={messages} />
                                {isTyping ? <div className='ml-2'><Lottie style={{ margin: '0px' }} width={70} options={defaultOptions} /></div> : <><div></div></>}
                            </div>
                        </div>
                    }
                    {/* input for messages */}
                ...
                <div className='h-full grid text-3xl text-white  place-content-center'>Click on user to start chatting</div>
            )}
        </div>
    );
}
